  ...
  function encode (schema, doc, toEncrypt) {
    ...
    for (var name in schema.paths) {
      ...
      if (schema.paths[name].instance === 'ObjectID' && val) {
        // Check for nested object path `some.path.id`
        if (!!~name.indexOf('.')) {
          nested(obj, name, method(val.toString()))
        // Direct path
        } else {
          obj[name] = method(val.toString())
        }
      }
      ...
      if (schema.paths[name].casterConstructor) {
        ...
        if (!!~schema.paths[name]
          ...
          .toString()
          .indexOf('ObjectId')) {
          for (var x = 0; x < obj[name].length; x++) {
            obj[name][x] = method(val[x].toString())
          }
        // Array of embedded schemas
          ...
          for (var x = 0; x < obj[name].length; x++) {
            obj[name][x] = encode(schema.paths[name].schema, val[x], toEncrypt)
          }
        }
      }
    }
